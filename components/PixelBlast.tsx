...
type PixelBlastProps = {
  ...
  rippleOnMove?: boolean;
  moveRippleThrottle?: number;
...
export default function PixelBlast({
  ...
  const lastMoveRippleRef = useRef(0);
  ...
  useEffect(() => {
    ...
    const triggerRipple = (fx: number, fy: number) => {
    ...
    const onPointerDown = (event: PointerEvent) => {
      const { fx, fy, inside } = mapToPixels(event);
    ...
    const onPointerMove = (event: PointerEvent) => {
      const { fx, fy, width, height, inside } = mapToPixels(event);
      ...
      if (enableRipples && rippleOnMove) {
        const now = performance.now();

        if (now - lastMoveRippleRef.current > moveRippleThrottle) {
          triggerRipple(fx, fy);
    ...
    const animate = () => {
      ...
      if (noiseEffect) {
        const timeUniform = noiseEffect.uniforms.get("uTime");
        if (timeUniform) timeUniform.value = uniforms.uTime.value;
      }
